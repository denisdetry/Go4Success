...
class TestRegisterUserView(APITestCase):
    ...
    def test_register_user_ok(self):
        ...
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
    ...
    def test_register_user_password_length_fail(self):
        ...
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, 'Le mot de passe doit contenir au moins 8 caractères')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_register_user_password_special_character_fail(self):
        data = json.dumps({
            ...
            "last_name": "user",
            "first_name": "test",
            # password should contain special character. This is what we are testing
            "password": "Testpassword12",
        })
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, 'Le mot de passe doit contenir au moins un caractère spécial')
    #     self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_register_user_password_uppercase_fail(self):
        data = json.dumps({
            ...
            "last_name": "user",
            "first_name": "test",
            # password should contain uppercase. This is what we are testing
            "password": "testpassword123_",
        })
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, 'Le mot de passe doit contenir au moins une majuscule')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_register_user_password_lowercase_fail(self):
        data = json.dumps({
            ...
            "last_name": "user",
            "first_name": "test",
            # password should contain lowercase. This is what we are testing
            "password": "TESTPASSWORD123_",
        })
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, 'Le mot de passe doit contenir au moins une minuscule')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_register_user_password_digit_fail(self):
        data = json.dumps({
            ...
            "last_name": "user",
            "first_name": "test",
            # password should contain digit. This is what we are testing
            "password": "TestPassword_",
        })
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, 'Le mot de passe doit contenir au moins un chiffre')
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)

    def test_register_user_email_already_exists(self):
        user = User.objects.create_user(
            username='testuser', email="testuser@gmail.com", password="Testpassword123_")
        data = json.dumps({
            "username": "testuser",
        ...
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, "choisir une autre adresse mail, celui-ci existe déjà")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
    ...
    def test_register_user_username_already_exists(self):
        ...
        response = self.client.post(
            reverse('register'), data, content_type='application/json')
        self.assertEqual(
            response.data, "choisir un autre nom d'utilisateur, celui-ci existe déjà")
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
